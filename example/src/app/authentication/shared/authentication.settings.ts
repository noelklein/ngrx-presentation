import { UserManagerSettings } from 'oidc-client';

/* tslint:disable */
export function createMicrosoftOpenIDSettings(
  tenant: string,
  clientId: string,
  redirectUrl: string,
  logoutRedirectUrl: string
): UserManagerSettings {
  return {
    authority: `https://login.microsoftonline.com/${tenant}`,
    client_id: clientId,
    redirect_uri: redirectUrl,
    silent_redirect_uri: redirectUrl,
    post_logout_redirect_uri: logoutRedirectUrl,
    // metadata defined here because login.microsoftonline.com doesn't allow CORS
    metadata: {
      issuer: `https://sts.windows.net/${tenant}/`,
      authorization_endpoint: `https://login.microsoftonline.com/${tenant}/oauth2/authorize`,
      userinfo_endpoint: `https://login.microsoftonline.com/${tenant}/openid/userinfo`,
      end_session_endpoint: `https://login.microsoftonline.com/${tenant}/oauth2/logout`,
      jwks_uri: 'https://login.microsoftonline.com/common/discovery/keys'
    },
    // keys retrieved from https://login.microsoftonline.com/common/discovery/keys because of CORS
    signingKeys: [
      {
        "kty": "RSA", "use": "sig", "kid": "YbRAQRYcE_motWVJKHrwLBbd_9s", "x5t": "YbRAQRYcE_motWVJKHrwLBbd_9s", "n": "vbcFrj193Gm6zeo5e2_y54Jx49sIgScv-2JO-n6NxNqQaKVnMkHcz-S1j2FfpFngotwGMzZIKVCY1SK8SKZMFfRTU3wvToZITwf3W1Qq6n-h-abqpyJTaqIcfhA0d6kEAM5NsQAKhfvw7fre1QicmU9LWVWUYAayLmiRX6o3tktJq6H58pUzTtx_D0Dprnx6z5sW-uiMipLXbrgYmOez7htokJVgDg8w-yDFCxZNo7KVueUkLkxhNjYGkGfnt18s7ZW036WoTmdaQmW4CChf_o4TLE5VyGpYWm7I_-nV95BBvwlzokVVKzveKf3l5UU3c6PkGy-BB3E_ChqFm6sPWw",
        "e": "AQAB",
        "x5c": [""]
      },
      {
        "kty": "RSA", "use": "sig", "kid": "I6oBw4VzBHOqleGrV2AJdA5EmXc", "x5t": "I6oBw4VzBHOqleGrV2AJdA5EmXc", "n": "oRcN8f34zyc04f1l-G4iff5SuR1QE245pzd8eEpWJOm5-qAGXgQbxpw7eIElYweG5f09L_gCCKTbR80b_sGcB2vv_RvRd246HObCtUjB4tDkmS6J-ut1LRwXdaInoT31WybV5hhpNwfjGTkY2Db-wOHNPTfHNeI-FrvgxwtLnRWBuqgEiHawpVwLsTn8YV7kdjAQ10L9R0j7z0fCZJp14U7lDGu5r5ViT1aG0xSQ4SOjyt0FkHTrM5inED9af1LHVFiM6sIEu-Wude-8m0CqWFoKpY5JdP79BZPkB61y8sFKLa3aRanKPz9BzW-ep7Pe99bqoDLNNoDpNtJv7yOXMw",
        "e": "AQAB",
        "x5c": [""]
      },
      {
        "kty": "RSA", "use": "sig", "kid": "RrQqu9rydBVRWmcocuXUb20HGRM", "x5t": "RrQqu9rydBVRWmcocuXUb20HGRM", "n": "kqE2PL51yFq3gfJtTHzoUq9sNyIXXLoAu40qihZKzmDkcd0qFJkITQYooSTE6kFJWeE1cN0PAk4lYyZHgFqI03CbM-gXslotiF065TH_xGq-I0mvWRz32l15-DZugtji8T04uLo9jWJE7qem6eE6cSlmpw4gabnIW2Chvi2KWe3ChVhOpiCh2v1wt8TO_QqnVMe6BNo91N8z7RtDEs-XhAWKZ3-y3eK7kGjNz5SPLjQ8ZRsAdSGYQTu_o5ygtjUiy1xqIHJz1F4u8WueQt5n6_DKNVuZw6E-VBWQWiQPAhDom9SVy12Rafc38_Tt2MTTXR0Xn1em6Tn-IWsLNwZc4w",
        "e": "AQAB",
        "x5c": [""]
      }
    ]
  };
}
